...
\vskip 0.2cm
\textnormal{{\fontsize{14}{16}\selectfont A Study on Speed Dating}}
\vskip 1cm
...
set.seed(67982193)
path <- "H:\\Stat 841\\gSpSmS_project\\speeddating.csv"
...
```{r, fig.align="center", out.width="75%", out.height="75%", echo=FALSE}
ratings_by_partner <- dplyr::select(speed_dating, gender, sincere_o, intelligence_o, 
                             attractive_o, funny_o, ambitious_o)
ratings_by_self <- dplyr::select(speed_dating, gender, sincere, intelligence,
...
```{r, fig.align="center", out.width="85%", out.height="85%", echo=FALSE}
# hobbies correlation
...
xg <- xgboost(data=as.matrix(set1.numeric[,-64]), label=as.numeric(set1.numeric[,64])-1, 
              ...
              nrounds=1000, num_class=2, objective="multi:softprob", verbose = FALSE)
...
1. Given a set of attributes pertaining to potential partners, we are able to predict whether the two people are ready to date each other with **86% accuracy** using **Random Forests** method.
2. Based on a person’s own preferences, and the attributes of the potential partner, we are able to predict whether the person will be interested in the opposite person using **XGBoost** and **KNN** methods. The accuracy of these predictions is lower (**76-79%**).
